...
const savedExampleScale = process.client
  ? localStorage.getItem(STORAGE_KEYS.EXAMPLE_SCALE)
  : null;
const exampleScale = ref(
  savedExampleScale
    ? parseInt(savedExampleScale)
    : DEFAULT_VALUES.EXAMPLE_SCALE,
);
...
const sendCanvasImageToBackend = async () => {
  try {
    ...
    const response = await MYAPI.post("/predict", formData, {
      headers: {
